...
SAMPLE_SEED = 2974165144
TIMEOUT_SECONDS = 1000.0
MEMORY_LIMIT_BYTES = 12 * 1024 ** 3
UATU_BASE_COMMIT = "11d2add23b8a2b722ad38e150478f029d8c9e6b4"
...
GOOD_VALIDATIONS = {"validated_sat_model", "matches_known_unsat", "matches_other_unsat", "matches_reference_unsat", "unresolved"}
...
def audit_reference(record, directory, actual_hashes, pair, frozen_manifest,
                    harness_hashes, input_info, reject, label):
    """Audit optional Glucose evidence without adding any measured score."""
    reference = record.get("reference")
    if reference is None:
        if any(name.startswith("glucose") for name in actual_hashes):
            reject("reference artifacts without reference metadata at " + label)
        return False, False
    if not isinstance(reference, dict):
        reject("reference must be an object at " + label)
        return False, False
    eligible = (record.get("benchmark", {}).get("expected") == "unknown"
                and all(isinstance(pair[s], dict) for s in SOLVERS))
    if eligible:
        outcomes = {pair[s].get("outcome") for s in SOLVERS}
        eligible = "unsat" in outcomes and bool(outcomes & {"timeout", "resource_limit"})
    if not eligible:
        reject("reference was not limited to otherwise unknown single-solver UNSAT at " + label)
    if record.get("reference_commit") != GLUCOSE_COMMIT or record.get("reference_source_manifest") != frozen_manifest:
        reject("pinned reference source mismatch at " + label)
    if not valid_digest(record.get("reference_binary_sha256")):
        reject("missing reference binary provenance at " + label)
    if record.get("reference_input_sha256") != input_info.get("cnf_sha256"):
        reject("reference input provenance mismatch at " + label)
    expected_build = 'make -C glucose-source/simp MROOT="$GITHUB_WORKSPACE/glucose-source" -j2'
    if record.get("reference_build_command") != expected_build:
        reject("unexpected reference build command at " + label)
    command = reference.get("command")
    command_valid = (isinstance(command, list) and len(command) == 4
                     and all(isinstance(arg, str) for arg in command))
    if command_valid:
        command_valid = (Path(command[0]).name == "glucose" and command[1] == "-verb=0"
                         and Path(command[2]).name == "input.cnf" and Path(command[3]).name == "glucose.model")
    if not command_valid:
        reject("unexpected reference command at " + label)
    if (reference.get("timeout_seconds") != TIMEOUT_SECONDS
            or reference.get("memory_limit_bytes") != MEMORY_LIMIT_BYTES
            or type(reference.get("cpu_affinity")) is not int
            or not isinstance(pair.get("uatu"), dict)
            or reference.get("cpu_affinity") != pair["uatu"].get("cpu_affinity")):
        reject("reference resource conditions mismatch at " + label)
    for key in ("wall_seconds", "user_seconds", "system_seconds", "max_rss_kib"):
        if not finite_nonnegative(reference.get(key)):
            reject("invalid reference %s at %s" % (key, label))
    if type(reference.get("external_timeout")) is not bool or type(reference.get("returncode")) is not int:
        reject("missing reference exit evidence at " + label)
    if reference.get("log_sha256") != actual_hashes.get("glucose.log") or "glucose.log" not in actual_hashes:
        reject("reference log digest mismatch at " + label)
    statuses = set()
    try:
        with (directory/"glucose.log").open(errors="replace") as stream:
            for line in stream:
                line = line.rstrip("\r\n")
                if line in ("s SATISFIABLE", "s UNSATISFIABLE", "SATISFIABLE", "UNSATISFIABLE"):
                    statuses.add(line)
    except OSError as error:
        reject("cannot inspect reference log at %s: %s" % (label, error))
    outcome = reference.get("outcome")
    validation = reference.get("validation")
    wall = reference.get("wall_seconds")
    on_time = (finite_nonnegative(wall) and wall <= TIMEOUT_SECONDS
               and reference.get("external_timeout") is False)
    if outcome == "unsat":
        valid = (on_time and reference.get("returncode") == 20
                 and statuses == {"s UNSATISFIABLE"} and validation == "corroborates_unknown_unsat")
        if not valid:
            reject("invalid reference UNSAT evidence at " + label)
        return valid, False
    if outcome == "sat":
        checked = reference.get("model_check")
        valid = (on_time and reference.get("returncode") == 10 and statuses == {"s SATISFIABLE"}
                 and validation == "validated_sat_model" and isinstance(checked, dict))
        if valid:
            valid = (type(checked.get("returncode")) is int and checked["returncode"] == 0
                     and isinstance(checked.get("output"), str)
                     and re.fullmatch(r"VALID: vars=\d+ clauses=\d+\n?", checked["output"]) is not None
                     and checked.get("model_sha256") == actual_hashes.get("glucose-check.model")
                     and "glucose-check.model" in actual_hashes
                     and checked.get("checker_sha256") == harness_hashes.get("checker_sha256")
                     and checked.get("input_sha256") == input_info.get("cnf_sha256"))
        try:
            normalized_hash = hashlib.sha256(b"SAT\n")
            with (directory/"glucose.model").open("rb") as stream:
                for chunk in iter(lambda: stream.read(1024*1024), b""):
                    normalized_hash.update(chunk)
            valid = valid and normalized_hash.hexdigest() == actual_hashes.get("glucose-check.model")
        except OSError:
            valid = False
        if not valid:
            reject("invalid reference SAT model evidence at " + label)
        return False, valid
    if outcome == "timeout":
        if validation != "unresolved" or not (reference.get("external_timeout") is True
                or finite_nonnegative(wall) and wall >= TIMEOUT_SECONDS):
            reject("invalid reference timeout evidence at " + label)
    elif outcome == "resource_limit":
        if validation != "unresolved" or not on_time or reference.get("returncode") != 0:
            reject("invalid reference resource-limit evidence at " + label)
    else:
        reject("reference execution error at " + label)
    return False, False
...
def collect(artifacts, solver_dir, config_dir, expected_source_commit=None,
    ...
    glucose_manifest = read_config("expected_glucose_source_manifest.json", dict, {})
    if not glucose_manifest or not all(valid_digest(value) for value in glucose_manifest.values()):
        reject("invalid pinned Glucose source manifest")
    ...
    build_commands = None
    binary_fingerprints = None
    reference_binary_fingerprint = None
    for index, (record, directory) in sorted(records.items()):
        ...
        for key, value in (("source_commit", expected_source_commit), ("github_run_id", expected_run_id),
                           ...
                           ("base_directory", "cpu/ver_4"), ("implementation", "Glucose 4.2.1 default sequential policies")):
        ...
        elif binary_fingerprints is None:
            binary_fingerprints = binaries
        elif binaries != binary_fingerprints:
            reject("inconsistent measured solver binaries at " + label)
        ...
        for name, value in artifact_manifest.items():
            if name not in ("uatu.log", "minisat.log", "minisat.model", "glucose.log", "glucose.model", "glucose-check.model") or not valid_digest(value):
        ...
        reference_unsat, reference_sat = audit_reference(record, directory, actual_hashes, pair,
                glucose_manifest, harness_hashes, input_info, reject, label)
        if record.get("reference") is not None:
            if reference_binary_fingerprint is None:
                reference_binary_fingerprint = record.get("reference_binary_sha256")
            elif record.get("reference_binary_sha256") != reference_binary_fingerprint:
                reject("inconsistent reference binaries at " + label)
        ...
        for solver in SOLVERS:
            ...
            compatible_validations = {
                ...
                "unsat": {"matches_known_unsat", "matches_other_unsat", "matches_reference_unsat", "unverified_unsat", "wrong_result"},
            ...
            elif validation == "matches_reference_unsat":
                answer_valid = expected_answer == "unknown" and reference_unsat
            elif validation == "unverified_unsat":
                other = pair["minisat" if solver == "uatu" else "uatu"]
                if reference_unsat or reference_sat:
                    reject("unverified UNSAT contradicts reference evidence for " + solver_label)
                elif (expected_answer == "unknown" and isinstance(other, dict)
    ...
    return {
        ...
        "reference_commit": GLUCOSE_COMMIT, "reference_source_manifest": glucose_manifest,
        ...
        "validation_definition": "SAT models checked against every original clause; UNSAT compared with known GBD status, the other measured solver, or pinned Glucose 4.2.1 for otherwise unknown single-solver UNSAT. Glucose verification occurs after both timed runs and is excluded from both PAR-2 scores. No UNSAT proof checking. Timeouts and memory limits are unresolved, not correctness passes.",
