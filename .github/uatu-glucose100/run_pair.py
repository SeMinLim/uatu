...
def run_solver(command, output, uatu, reference=False):
    ...
    outcome, resource_reason = classify_result(command, uatu, code, timed_out, elapsed, text, reference)
    counters = {}
    if uatu:
...
def classify_result(command, uatu, code, timed_out, elapsed, text, reference=False):
    lines = text.splitlines()
    if timed_out or elapsed > TIMEOUT:
        return 'timeout', None
    if reference:
        statuses = {line for line in lines if line in ('s SATISFIABLE', 's UNSATISFIABLE', 'SATISFIABLE', 'UNSATISFIABLE')}
        if code == 10 and statuses == {'s SATISFIABLE'}:
            return 'sat', None
        if code == 20 and statuses == {'s UNSATISFIABLE'}:
            return 'unsat', None
        if code == 0 and not statuses and 'INDETERMINATE' in lines:
            return 'resource_limit', 'Pinned Glucose OutOfMemoryException diagnostic, simp/Main.cc lines 301-304'
...
def check_model(checker, cnf, model, format_name):
    ...
    return {'returncode': checked.returncode, 'output': checked.stdout,
            'model_sha256': sha256(model), 'checker_sha256': sha256(HERE/'validate_model.cpp'),
            'input_sha256': sha256(cnf)}


def corroborate_unsat(glucose_dir, cnf, checker, out, result):
    """Run a third solver only as answer evidence, after both measured runs."""
    assert capture(['git', '-C', str(glucose_dir), 'rev-parse', 'HEAD']) == GLUCOSE_COMMIT
    assert not capture(['git', '-C', str(glucose_dir), 'diff', '--name-only', 'HEAD'])
    expected = json.loads((HERE/'expected_glucose_source_manifest.json').read_text())
    manifest = {str(path.relative_to(glucose_dir)): sha256(path)
    ...
    result.update(reference_commit=GLUCOSE_COMMIT, reference_source_manifest=manifest,
                  reference_binary_sha256=sha256(glucose),
                  reference_build_command='make -C glucose-source/simp MROOT="$GITHUB_WORKSPACE/glucose-source" -j2',
                  reference_input_sha256=sha256(cnf),
                  reference_purpose='Evidence only for unknown UNSAT; excluded from both PAR-2 scores')
    command = [str(glucose), '-verb=0', str(cnf), str(out/'glucose.model')]
    result['reference'] = {'command': command, 'validation': 'pending'}
    write_json(out/'result.json', result)
    reference = run_solver(command, out/'glucose.log', False, reference=True)
    result['reference'] = reference
    if reference['outcome'] == 'unsat':
        reference['validation'] = 'corroborates_unknown_unsat'
...
def main():
    ...
    parser.add_argument('--glucose-dir', type=Path, default=REPO/'glucose-source')
    ...
    if (out/'result.json').exists():
        raise RuntimeError('result.json already exists; use a new output directory to preserve the attempt')
    work = out/'work'
    ...
    assert not capture(['git','-C',str(minisat_dir),'diff','--name-only','HEAD'])
    ...
    try:
        ...
        outcomes = {result[solver]['outcome'] for solver in ('uatu', 'minisat')}
        if item['expected'] == 'unknown' and 'unsat' in outcomes and outcomes & {'timeout', 'resource_limit'}:
            corroborate_unsat(args.glucose_dir.resolve(), cnf, checker, out, result)
        for solver in ['uatu','minisat']:
            ...
            elif result.get('reference', {}).get('validation') == 'corroborates_unknown_unsat':
                record['validation']='matches_reference_unsat'
            elif result.get('reference', {}).get('validation') == 'validated_sat_model':
                record['validation']='wrong_result'
                record['contradiction']='UNSAT contradicts the original-CNF SAT model from pinned Glucose'
            else:
                record['validation']='unverified_unsat'
        good={'validated_sat_model','matches_known_unsat','matches_other_unsat','matches_reference_unsat','unresolved'}
