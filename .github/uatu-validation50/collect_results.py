#!/usr/bin/env python3
"""Audit the frozen 50-instance SAT 2025 correctness experiment artifacts."""
...
def collect(artifacts, solver_dir, config_dir, expected_source_commit=None,
    ...
    if len(sample) != SAMPLE_COUNT:
        reject("expected 50 sampled instances, got %d" % len(sample))
    for item in sample:
    ...
    if set(expected) != set(range(SAMPLE_COUNT)) or len(set(sample_hashes)) != SAMPLE_COUNT:
        reject("sample must contain indices 0 through 49 and 50 distinct hashes")
    ...
    if len(raw_records) > SAMPLE_COUNT:
        reject("expected exactly 50 result files, got %d" % len(raw_records))
    ...
    for index, (record, directory) in sorted(records.items()):
        ...
        if record.get("source_directory") not in ("cpu/ver_5", ".github/uatu-validation50/candidate"):
    ...
    return {
        ...
        "source_commit": expected_source_commit, "github_run_id": expected_run_id,
...
def main():
    ...
    print(json.dumps({key: summary[key] for key in
                      ("status", "sample_count", "record_count", "counts", "validation_counts", "errors")}, indent=2))
